...
def create_keycloak_user(username, email, first_name, last_name, password):
    ...
    logger.info(
        "create_keycloak_user called with username=%s, email=%s", username, email
    )
    ...
    try:
        logger.debug(
            "Sending POST to Keycloak: url=%s, headers=%s, payload=%s",
            url,
            headers,
            payload,
        )
        ...
        logger.debug(
            "Keycloak response: status=%s, text=%s", resp.status_code, resp.text
        )
    ...
    if resp.status_code != 201:
        logger.error(
            "Keycloak user creation failed: status=%s, body=%s",
            resp.status_code,
            resp.text,
        )
    try:
